...
function replaceFiles(cliente, objFile) {
  const path_file = pathAdd(`../config_setup/${objFile.file_name}`);
  let fileData = fs.readFileSync(path_file, 'utf-8');
...
function moveFiles(cliente, objFile) {
  const path_file = pathAdd(`../config_setup/${objFile.file_name}_${cliente}.${objFile.extension}`);
  const fileData = fs.readFileSync(path_file, 'utf-8');
