class FamiliaEntidadContable < ApplicationRecord
  ...
  def self.create_update_familia_entidad_contable(params, is_save=false)
    res                                = Response.new

    FamiliaEntidadContable.transaction do

      tipo_articulo                    = FamiliaEntidadContable.where(:id => params[:id]).first_or_create

      tipo_articulo.descripcion        = params[:descripcion]
      tipo_articulo.entidad            = params[:entidad]
      ...
      if result_procesos.status_valid && tipo_articulo.errors.empty? && (!is_save || (is_save && tipo_articulo.save!))
        ...
        action = params['id'] ? 'actualizado' : 'creado'
				res.add_msg("Categoria de #{CategoriaEntidadContable.get_tipo_plural(params[:entidad])} #{action} correctamente.")
