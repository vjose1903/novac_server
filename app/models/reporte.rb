...
# Creando un hash con las siguientes claves:
    ...
    def self.get_cuentas_cobrar(params)
      ...
      .select(select_).where(query).where("cabecera_facturas.balance >= 1").group(group_by).where("cabecera_facturas.tipo_factura_id != #{TiposFacturasId.pre_factura}")
    ...
    def self.get_cuentas_con_pagos(params)
      ...
      query['tipo_factura_id']    = TiposFacturasId.pre_factura
    ...
    def self.calcular_cantidad_proporcional(productos)
      total_venta=0
      plural = { Quintal: 'Quintales', Libra: 'Libras', Caja: 'Cajas', Paquete: 'Paquetes', Unidad: 'Unidades', Saco: 'Sacos', Funda: 'Fundas', Galon: 'Galones', Producto: 'Productos' }
    ...
    def self.get_ventas(params)
      ...
      .select(select_).where(query).where(where_formas).where(query_is_viaje).where("cabecera_facturas.tipo_factura_id != #{TiposFacturasId.pre_factura}")
