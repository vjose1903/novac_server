class PagoFactura < ApplicationRecord
  ...
  validates :total,    presence: { :message => "El pagoi no esta completado." }, numericality: { greater_than: 0, :message => "El total del pago debe de ser mayor a 0." }
  ...
  def self.models_includes
    includes = [
      ...
      :tipo_factura,
      {pago_factura_detalles: [:pago_facturas, :cabecera_factura]},
    ]
    return includes
  end
