class Trabajo < ApplicationRecord
  ...
  def self.parsearTrabajosFiltro(trabajos)
    trabajos.each do |work|
      ...
      if work["cancelado"]
        work["actualmente"] = { estado: "Cancelado", color: "red" }
      else
        if !work["empezado"] && !work["terminado"]
          work["actualmente"] = { estado: "En espera", color: "grey" }
        elsif work["empezado"] && !work["terminado"]
          work["actualmente"] = { estado: "Empezado", color: "yellow" }
        elsif work["empezado"] && work["terminado"]
          work["actualmente"] = { estado: "Terminado", color: "green" }
        end
      end
