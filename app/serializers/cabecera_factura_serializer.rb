class CabeceraFacturaSerializer < ActiveModel::Serializer
	...
	attribute :recibos,                                        if: Proc.new { self.get_param('all') || self.get_param('recibos') }
	...
	def suplidor
		...
		unless object.suplidor.blank?
			...
			suplidor[:nombre]            = object.suplidor.nombre_completo
	...
	def notas
		...
		if object.tiene_nota
			...
			notas = serialize_parser(notas, { fecha_equivalente: true, numero_comprobante: true, id: true, user_id: true, detalles_facturas_notas: true, total: true, tipo: true, tipo_label:true })
		end
		notas
	end
