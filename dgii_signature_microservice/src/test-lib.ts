...
import { crearArchivoXML, leerArchivo, sleep } from "./utils/functions";
...
async function firmarXML(fileObj: { RNCEmisor: string; noEcf: string; file: string }, index: number) {
	try {
		const xmlPath = path.resolve( __dirname, `utils/paso-2/sin_firmar/${fileObj.file}` );
		...
		crearArchivoXML(formattedXml, path.resolve( __dirname, `utils/paso-2/firmados/${fileName}` ));
...
function saveResponse(filename: string, response: any, index: number) {
	const resultsPath = path.resolve( __dirname, `utils/paso-2/firmados/results.txt` );
...
function getXMLS(): Promise< { RNCEmisor: string; noEcf: string; file: string }[] > {
	return new Promise((resolve, reject) => {
		const directoryPath = path.resolve(__dirname, "utils/paso-2/sin_firmar/");
