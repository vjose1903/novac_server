namespace :db do
  ...
  task backup: :environment do
    ...
    archive_path      = "#{Rails.root}/db/panaderia_brendy_#{rails_env.downcase}_#{timestamp}.sql"

    ENV['PGPASSWORD'] = tulu

    # pg_dump           = "pg_dump --verbose --format=c --inserts -U novacSystem -h db-dev --dbname=panaderia_brendy_#{rails_env.downcase} -f #{archive_path}"
    pg_dump           = "pg_dump --verbose --format=c --inserts -U novacSystem -h db-prod --dbname=panaderia_brendy_#{rails_env.downcase} -f #{archive_path}"
